...
export default async function CamerasPage() {
  ...
  return <div className="grid gap-8">
    ...
    {(cameras ?? []).length === 0 ? <Card><CardContent className="py-10 text-center text-sm text-muted-foreground">{t("empty")}</CardContent></Card> : <div className="grid gap-4 md:grid-cols-2">{(cameras ?? []).map((camera) => <CameraCard key={camera.id} camera={camera as CameraWithProperty} properties={properties ?? []} locale={locale} labels={getFieldLabels(t)} text={{ open: t("open"), photo: t("photo"), streamConfigured: t("streamConfigured"), appAccess: t("appAccess"), stale: t("stale"), edit: t("edit"), delete: t("delete"), justNow: t("relative.justNow"), minutesAgo: t.raw("relative.minutesAgo"), hoursAgo: t.raw("relative.hoursAgo"), daysAgo: t.raw("relative.daysAgo") }} />)}</div>}
  </div>;
}

type FieldLabels = ReturnType<typeof getFieldLabels>;
type CardText = { open: string; photo: string; streamConfigured: string; appAccess: string; stale: string; edit: string; delete: string; justNow: string; minutesAgo: string; hoursAgo: string; daysAgo: string };
...
function CameraCard({ camera, properties, locale, labels, text }: { camera: CameraWithProperty; properties: { id: string; name: string }[]; locale: string; labels: FieldLabels; text: CardText }) {
  ...
  return <Card><CardHeader className="pb-3"><div className="flex items-start justify-between gap-3"><div><CardTitle className="flex items-center gap-2"><Camera className="size-5" />{camera.name}</CardTitle><CardDescription>{camera.property?.name}{camera.location ? ` · ${camera.location}` : ""}</CardDescription></div>{camera.access_url && <Button size="sm" variant="outline" render={<a href={camera.access_url} target="_blank" rel="noreferrer" />}><ExternalLink className="size-4" /> {text.open}</Button>}</div></CardHeader><CardContent className="grid gap-3 text-sm">{camera.snapshot_url && <CameraPreview url={camera.snapshot_url} name={camera.name} lastSnapshotAt={camera.last_snapshot_at} />}<p className={freshnessClass}>{camera.provider}{camera.last_snapshot_at ? ` · ${text.photo} ${relTime(ageMinutes ?? 0, text)} · ${capturedAt}` : camera.stream_url ? ` · ${text.streamConfigured}` : ` · ${text.appAccess}`}{ageMinutes !== null && ageMinutes > 60 ? ` · ${text.stale}` : ""}</p>{camera.notes && <p>{camera.notes}</p>}<details><summary className="cursor-pointer text-muted-foreground hover:text-foreground">{text.edit}</summary><CameraEditForm><input type="hidden" name="id" value={camera.id} /><CameraFields camera={camera} properties={properties} labels={labels} /></CameraEditForm></details><form action={deleteCamera}><input type="hidden" name="id" value={camera.id} /><input type="hidden" name="property_id" value={camera.property_id} /><Button type="submit" size="sm" variant="ghost" className="w-fit text-destructive hover:text-destructive">{text.delete}</Button></form></CardContent></Card>;
}
