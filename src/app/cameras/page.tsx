...
function CameraCard({ camera, properties, locale, labels, text }: { camera: CameraWithProperty; properties: { id: string; name: string }[]; locale: string; labels: FieldLabels; text: CardText }) {
  ...
  return <Card><CardHeader className="pb-3"><div className="flex items-start justify-between gap-3"><div><CardTitle className="flex items-center gap-2"><Camera className="size-5" />{camera.name}</CardTitle><CardDescription>{camera.property?.name}{camera.location ? ` · ${camera.location}` : ""}</CardDescription></div>{camera.access_url && <Button size="sm" variant="outline" render={<a href={camera.access_url} target="_blank" rel="noreferrer" />}><ExternalLink className="size-4" /> {text.open}</Button>}</div></CardHeader><CardContent className="grid gap-3 text-sm">{camera.snapshot_url && <CameraPreview cameraId={camera.id} url={camera.snapshot_url} name={camera.name} lastSnapshotAt={camera.last_snapshot_at} lastSyncedAt={camera.last_synced_at} captureRequestedAt={camera.capture_requested_at} />}<div><p className={freshnessClass}>{camera.provider}{camera.last_snapshot_at ? ` · ${text.photo} ${relTime(ageMinutes ?? 0, text)} · ${capturedAt}` : camera.stream_url ? ` · ${text.streamConfigured}` : ` · ${text.appAccess}`}{ageMinutes !== null && ageMinutes > 60 ? ` · ${text.stale}` : ""}</p>{showSyncStatus && syncAgeMinutes !== null && <p className="mt-1 text-xs text-muted-foreground">{text.syncStatus.replace("{capture}", relTime(ageMinutes ?? 0, text)).replace("{sync}", relTime(syncAgeMinutes, text))}</p>}</div>{camera.notes && <p>{camera.notes}</p>}<details><summary className="cursor-pointer text-muted-foreground hover:text-foreground">{text.edit}</summary><CameraEditForm><input type="hidden" name="id" value={camera.id} /><CameraFields camera={camera} properties={properties} labels={labels} /></CameraEditForm></details><form action={deleteCamera}><input type="hidden" name="id" value={camera.id} /><input type="hidden" name="property_id" value={camera.property_id} /><Button type="submit" size="sm" variant="ghost" className="w-fit text-destructive hover:text-destructive">{text.delete}</Button></form></CardContent></Card>;
}
