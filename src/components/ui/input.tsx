...
function Input({ className, type, ...props }: React.ComponentProps<"input">) {
  return (
    <InputPrimitive
      ...
      data-slot="input"
      className={cn(
        // WIK-128: h-8 → h-10 (comfortable touch target on mobile),
        // rounded-lg → rounded-xl (matches button), more horizontal
        // padding, and removed the md:text-sm downgrade so text stays
        // legible on desktop too. Subtle shadow on focus replaces the
        // default ring-only feedback.
